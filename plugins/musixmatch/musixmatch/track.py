...
class Track(object):
    ...
    def __str__(self):
        """ pretty printout """
        return 'MusixMatch Track: ' + string_(self.__dict__)
    ...
    for k in list(args.keys()):
        if not k in valid_params:
            raise util.MusixMatchAPIError(-1,
                                          'Invalid track search param: ' + string_(k))
    # call and gather a list of tracks
    track_list = list()
    params = dict((k, v) for k, v in args.items() if not v is None)
...
def chart(**args):
    ...
    for k in list(args.keys()):
        if not k in valid_params:
            raise util.MusixMatchAPIError(-1, 'Invalid chart param: ' + string_(k))
