...
class PartLevels():
    ...
    def make_annotations(self, release_id, track, wid):
        ...
        if options['cwp_key_include'] and 'key' in self.parts[wid] and self.parts[wid]['key']:
            ...
            while work_chk in self.works_cache:
                ...
                if parent_chk in self.parts and 'name' in self.parts[parent_chk]:
                    work_name = list_to_str(
                        self.parts[parent_chk]['name']) + ': ' + work_name
                work_chk = parent_chk
            ...
            if '~cwp_part_levels' in tm:
                ...
                for n in range(0, part_levels + 1):
                    if '~cwp_work_' + \
                        ...
                        source_id = list(
                            interpret(tm['~cwp_workid_' + str(n)]))
                        if n == 0:
                            self.append_tag(
    ...
    def get_text_tuples(self, release_id, track, text_type):
        ...
        Note that code in this method refers to synonyms (as that was written first), but applies eqaully to replacements
        :return:
        """
        tm = track.metadata
        strsyns = re.split(r'(?<!\\)/',
