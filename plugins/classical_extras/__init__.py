...
<br /><br />
4. Genres etc.: Options are available to customise the source and display of information relating to genres, 
        ...
        if release_id not in log_files:
            try:
                if release_id == 'session':
                    log_file = open(os.path.join(log_dir, filename), 'w', encoding='utf8', buffering=1)
                    # buffering=1 so that session log (low volume) is up to date even if not closed
                else:
                    log_file = open(os.path.join(log_dir, filename), 'w', encoding='utf8')  #, buffering=1)
...
def close_log(release_id, caller):
    ...
    if release_id in release_status:
        ...
        artists_time = release_status[release_id]['artists-done'] - release_status[release_id]['start']
        works_time = release_status[release_id]['works-done'] - release_status[release_id]['start']
    ...
    if 'session' in log_files and release_id in release_status:
        ...
        write_log('session', 'basic', 'Duration = %s. Artists time = %s. Works time = %s. Of which: Lookup time = %s. '
                                      'Album-process time = %s. Number of lookups = %s.',
                  duration, artists_time, works_time, lookup_time, album_process_time, lookups)
    if release_id in release_status:
    ...
    if isinstance(obj, list):
        ...
        for i, item in enumerate(obj):
            if isinstance(item, XmlNode):
                item = item.__dict__
            if INFO:
                write_log(release_id, 'info', 'Getting response for list item no.%s of %s - object is: %s', i+1, objlen, item)
            parse_data(release_id, item, response_list, *match)
            if INFO:
                write_log(release_id, 'info', 'response_list for list item no.%s of %s is %s', i+1, objlen, response_list)
        ...
        if match[0] in obj:
            if len(match) == 1:
                ...
                if INFO:
                    write_log(release_id, 'info', 'response_list (last match item): %s', response_list)
            else:
                match_list = list(match)
                match_list.pop(0)
                parse_data(release_id, obj[match[0]], response_list, *match_list)
                if INFO:
                    write_log(release_id, 'info', 'response_list (passing up): %s', response_list)
            return response_list
        elif ':' in match[0]:
            test = match[0].split(':')
            match2 = test[0].split('.')
            ...
            if INFO:
                write_log(release_id, 'info', 'Value comparison - looking in %s for value %s', test_data, test[1])
            if len(test) > 1:
                if (test[1] in test_data) or ((test[1]=='True') in test_data):  # latter is because Booleans are stored as such, not as strings, in JSON
                    if len(match) == 1:
                        response = obj
            ...
            if INFO:
                write_log(release_id, 'info', 'response_list (from value look-up): %s', response_list)
            return response_list
        else:
            if 'children' in obj:
                parse_data(release_id, obj['children'], response_list, *match)
            if INFO:
                write_log(release_id, 'info', 'response_list (from children): %s', response_list)
            return response_list
    else:
        if INFO:
            write_log(release_id, 'info', 'response_list (obj is not a list or dict): %s', response_list)
        return response_list
...
def get_references_from_file(release_id, path, filename):
    ...
    try:
        ...
        if options['cwp_muso_genres'] or options['cwp_muso_classical'] or options['cwp_muso_dates'] or options['cwp_muso_periods']:
            write_log(release_id, 'error', 'File %s does not exist or is corrupted', os.path.join(path, filename))
    finally:
                                ...
                                for ea_opt in plugin_options(section) + addn:
                                    ...
                                    if ea_opt['name'] == opt_det:
                                        ...
                                        if options[ea_opt['option']
                                            ...
                                            append_tag(release_id, tm, '003_information:options_overridden', str(
        ...
        if orig_metadata:
            ...
            if options['cwp_genres_use_file']:
                if 'genre' in orig_metadata:
                    append_tag(release_id, tm, '~cwp_candidate_genres', orig_metadata['genre'])
            ...
            really_keep_list.append(options['cwp_options_tag'] + ':workparts_options')
            really_keep_list.append(options['cea_options_tag'] + ':artists_options')
            for tagx in keep_list:
                tag = tagx.strip()
                really_keep_list.append(tag)
                write_log(release_id, 'info', 'really_keep_list = %s', really_keep_list)
...
def get_aliases(self, release_id, album, options, releaseXmlNode):
    ...
    if 'artist_locale' in config.setting and options['cea_aliases'] or options['cea_aliases_composer']:
        ...
        for m in media:
            ...
            # write_log(release_id, 'info', 'tracks for aliases etc = %s', tracks)   # TODO remove debug when OK
            for track in tracks:
                for t in track:
        ...
        # Release artists
        get_aliases_and_credits(self, options, release_id, album, releaseXmlNode, lang,
                                options['cea_credited'])
        # Next bit needed to identify artists who are album artists
        self.release_artists_sort[album] = parse_data(release_id, releaseXmlNode, [], 'artist-credit',
                                                      'artist', 'sort-name')
        # Release relationship artists
        get_relation_credits(self, options, release_id, album, releaseXmlNode, lang,
        ...
        for m in media:
            ...
            for track in tracks:
                for t in track:
                    ...
                    # write_log(release_id, 'info', 'get recording artist relationships')  # TODO remove debug when OK
                    obj = parse_data(release_id, t, [], 'recording')
                    get_relation_credits(self, options, release_id, album, obj, lang, options['cea_recording_relationship_credited'])
                    # Track artists
...
def get_artists(options, release_id, tm, relations, relation_type):
    ...
    if options['log_debug'] or options['log_info']:
        write_log(release_id, 'debug', 'In get_artists. relation_type: %s, relations: %s', relation_type, relations)
    log_options = {
    ...
    for artist_type in artist_types:
        type_list = parse_data(release_id, relations, [], 'target-type:artist', 'type:' +
                               artist_type)
        for type_item in type_list:
            artist_name_list = parse_data(release_id, type_item, [], 'artist', 'name')
            artist_sort_name_list = parse_data(release_id, type_item, [], 'artist', 'sort-name')
            if artist_type not in [
                ...
                instrument_list_list = parse_data(release_id, type_item, [], 'attributes')
                if instrument_list_list:
                    instrument_list = instrument_list_list[0]
                else:
                    instrument_list = []
                credited_inst_list = instrument_list[:]
                credited_inst_dict_list = parse_data(release_id, type_item, [], 'attribute-credits')  # keyed to insts
                ...
                if log_options['log_info']:
                    ...
                    write_log(release_id, 'info', 'credited_inst_dict = %s', credited_inst_dict)
                for i, inst in enumerate(instrument_list):
                    if inst in credited_inst_dict:
                        credited_inst_list[i] = credited_inst_dict[inst]
                # for inst_node in att_list:
                #     # cred_list = parse_data(release_id, inst_node, [], 'credited-as')  # TODO Need to fix
                #     cred_list = []  # temp fix
                #     inst_list = inst_node  # parse_data(release_id, inst_node, [], 'text')
                #     if cred_list:
...
def apply_artist_style(options, release_id, lang, a_list, name_style, name_tag, sort_tag, names_tag, names_sort_tag):
    ...
    write_log(release_id, 'info', 'a_list = %s', a_list)  # TODO temp - remove
    for a_item in a_list:
        for acs in a_item:
            ...
            names = {}
            if lang:
                names['alias'] = parse_data(release_id, acs, [], 'artist', 'aliases', 'locale:' +
...
def replace_roman_numerals(s):
    ...
    p = re.compile(
        ...
        # Matches Roman numerals (+ ensure non-Latin chars treated as word chars)
    romans = p.findall(s)
    if not romans:
        p_lower = re.compile(
            r'\b(m{0,4}(cm|cd|d?)?c{0,3}(xc|xl|l?)?x{0,3}(ix|iv|v?)?i{0,3})\b(\.|:|,|;|$)',)
        # try lower case (but don't mix them)
        romans = p_lower.findall(s)
    for roman in romans:
        if roman[0]:
...
def map_tags(options, release_id, album, tm):
    ...
    if options['cea_no_lyricists'] and not any([x for x in str_to_list(tm['~cea_performers']) if 'vocals' in x]):
        if 'lyricist' in tm:
            del tm['lyricist']
        ...
        if options['cwp_use_muso_refdb'] and options['cwp_muso_periods'] and PERIOD_DICT:
            ...
            for period in PERIODS:
                if PERIODS[period][0].lstrip('-').isdigit() and PERIODS[period][1].lstrip('-').isdigit():
                    ...
                    PERIODS[period] = (9999, 'ERROR - start and/or end of ' + period + ' are not integers')
            ...
            for p in periods:
                ...
                if len(p) == 3:
                    ...
                    PERIODS[p[0]] = (9999, 'ERROR in period map - each item must contain 3 elements')
    if options['cwp_period_tag'] and PERIODS:
        ...
        sorted_periods = collections.OrderedDict(sorted(PERIODS.items(), key=lambda t: t[1]))
        for period in sorted_periods:
            if isinstance(sorted_periods[period][1],str) and 'ERROR' in sorted_periods[period][1]:
                tm[options['cwp_period_tag']] = ''
                append_tag(release_id, tm, '001_errors:9', '9. ' + sorted_periods[period])
                break
    ...
    if name_credit_list:
        for name_credits in name_credit_list:
            for name_credit in name_credits:
                ...
                if credited_artist:
                    name = parse_data(release_id, name_credit, [], 'artist', 'name')
                    sort_name = parse_data(release_id, name_credit, [], 'artist', 'sort-name')
...
def get_relation_credits(self, options, release_id, album, obj, lang, credited):
    """
    :param release_id: name for log file - usually =musicbrainz_albumid
        unless called outside metadata processor
    :param self:
    ...
    """
    # write_log(release_id, 'info', 'in get_relations_credits.')  # TODO remove debug when OK
    credited_artists = None
    aliases = None
    # write_log(release_id, 'info', 'obj = %s', obj)  # TODO remove debug when OK
    rels = parse_data(release_id, obj, [], 'relations', 'target-type:work',
                    'work', 'relations', 'target-type:artist')
    # write_log(release_id, 'info', 'rels = %s', rels)  # TODO remove debug when OK
    # for rel in rels:
    for artist in rels:
        sort_names = parse_data(release_id, artist, [], 'artist', 'sort-name')
        if sort_names:
            credited_artists = parse_data(release_id, artist, [], 'target-credit')
            if credited_artists and credited_artists[0] != '' and credited:
                self.artist_credits[album][sort_names[0]
                                               ] = credited_artists[0]
            aliases = parse_data(release_id, artist, [], 'artist', 'aliases', 'locale:' +
                                 lang, 'primary:True', 'name')
            if aliases:
                self.artist_aliases[sort_names[0]] = aliases[0]
        # if credited_artists:                                          # TODO remove debug when OK
        #     ca = credited_artists[0]
        # else:
        #     ca = "NONE"
        # if aliases:
        #     al = aliases[0]
        # else:
        #     al = "NONE"
        # write_log(release_id, 'info', 'in get_relations_credits pt1. credited_artists = %s', ca)
        # write_log(release_id, 'info', 'in get_relations_credits pt1. aliases = %s', al)
    ...
    for artist in rels2:
        ...
        # else:
        #     al = "NONE"
        # write_log(release_id, 'info', 'in get_relations_credits pt2. credited_artists = %s', ca)
        # write_log(release_id, 'info', 'in get_relations_credits pt2. aliases = %s', al)
    ...
    def add_artist_info(
        ...
        if not (
            ...
            if tm['discnumber'] == '1' and tm['tracknumber'] == '1':
                ...
                relation_list = parse_data(release_id, releaseXmlNode, [], 'relations')
            ...
            track_artist_list = parse_data(release_id, trackXmlNode, [], 'artist-credit')
            ...
            if 'recording' in trackXmlNode:
                ...
                for record in recording:
                    ...
                    recording_artist_list = parse_data(release_id, record, [], 'artist-credit')
                    ...
                    # xml_type = 'recording'
                    relation_list = parse_data(release_id, record, [], 'relations')
                    ...
                    if not options['classical_work_parts']:
                        work_artist_list = parse_data(release_id, record, [], 'relations',
                                                      'target-type:work',
...
class PartLevels():
    ...
    def __init__(self):
        ...
        # contains list of workIds which are descendants of a given workId, to prevent recursion when adding new ids
    ...
    def add_work_info(
        ...
        self.synonyms[track] = self.get_synonyms(release_id, track)
        ...
        if workIds and not (options["ce_no_run"] and (
            ...
            for key in sorted(keyed_workIds):
                ...
                work_rels = parse_data(release_id, trackXmlNode, [], 'recording', 'relations',
                                       'target-type:work', 'work.id:' + workId)
                if self.INFO:
                    write_log(release_id, 'info', 'work_rels: %s', work_rels)
                work_attributes = parse_data(release_id, work_rels, [], 'attributes')[0]
                if self.INFO:
                    write_log(release_id, 'info', 'work_attributes: %s', work_attributes)
                work_titles = parse_data(release_id, work_rels, [], 'work', 'title')
                work_list_info_item = {
                    'id': workId,
                ...
                for wid in wid_list:  # wid is a tuple
                    ...
                    if wid in self.parts:
                        ...
                        if parentList:
                            ...
                            # movements listed in the correct order (i.e. ordering-key, if available)
                            if len(wid) > 1:
                                for idx in wid:
                                    if idx == workId:
                                        match_tree = [
                                        ...
                                        if self.INFO:
                                            write_log(release_id, 'info', 'multi-works - ordering key for id %s is %s', idx,
                                                      parse_result)
                                        if parse_result:
                                            if isinstance(parse_result[0], str) and parse_result[0].isdigit():
                                                key = int(parse_result[0])
                            ...
                            # remove any parents that are descendants of wid as they will result in circular references
                            del_list =[]
                            for i, parentId in enumerate(parentIds):
                                for work_item in wid:
                                    if work_item in self.child_listing and parentId in self.child_listing[work_item]:
                                        del_list.append(i)
                            for i in del_list:
                                removed_id = parentIds.pop(i)
                                removed_name = parents.pop(i)
                                if self.ERROR or self.INFO:
                                    write_log(release_id, 'error', "Found parent which is descendant of child - "
                                                                   "not using, to prevent circular references. id = %s,"
                                                                   " name = %s",
                                              removed_id, removed_name)
                                tm = track.metadata
    ...
    def work_process_metadata(self, release_id, workId, wid, track, response):
        ...
        self.parts[wid]['worktype_genres'] = parse_data(release_id, response, [], 'type')
        key = parse_data(release_id, response, [], 'attributes', 'type:Key', 'value')
        self.parts[wid]['key'] = key
        composed_begin_dates = year(
            parse_data(release_id, response, [], 'relations', 'target-type:artist',
                       'type:composer', 'begin'))
        ...
        if composed_begin_dates == composed_end_dates:
            ...
            composed_dates = list(zip(composed_begin_dates, composed_end_dates))
        ...
        published_begin_dates = year(
            parse_data(release_id, response, [], 'relations', 'target-type:label',
                       'type:publishing', 'begin'))
        ...
        if published_begin_dates == published_end_dates:
            ...
            published_dates = list(zip(published_begin_dates, published_end_dates))
        ...
        premiered_begin_dates = year(
            parse_data(release_id, response, [], 'relations', 'target-type:place',
                       'type:premiere', 'begin'))
        ...
        if premiered_begin_dates == premiered_end_dates:
            ...
            premiered_dates = list(zip(premiered_begin_dates, premiered_end_dates))
        ...
        if 'artist_locale' in config.setting:
            ...
            user_tags = parse_data(release_id, response, [], 'user-tags', 'name')
    ...
    def work_process_relations(self, release_id, track, workId, wid, relations):
        ...
        new_workIds = []
        new_works = []
        relation_attributes = parse_data(release_id, relations, [], 'target-type:work',
                                        'type:parts', 'direction:backward', 'attributes')
        new_work_list = []
        for relation_attribute in relation_attributes:
            if ('part of collection' not in relation_attribute) or options['cwp_collections']:
                new_work_list += parse_data(release_id, relations, [], 'target-type:work',
                                           'type:parts', 'direction:backward', 'work')
            if ('part of collection' in relation_attribute) and not options['cwp_collections'] and self.INFO:
                write_log(release_id, 'info',
                        'Not getting parent work because relationship is "part of collection" and option not selected')
        if new_work_list:
            if self.INFO:
                write_log(release_id, 'info', 'new_work_list: %s', new_work_list)
            new_workIds = parse_data(release_id, new_work_list, [], 'id')
            new_works = parse_data(release_id, new_work_list, [], 'title')
        else:
            arrangement_of = parse_data(release_id, relations, [], 'target-type:work',
                                        'type:arrangement', 'direction:backward', 'work')
            if arrangement_of and options['cwp_arrangements']:
                ...
                medley_of = parse_data(release_id, relations, [], 'target-type:work',
                                       'type:medley', 'work')
                direction = parse_data(release_id, relations, [], 'target-type:work',
                                       'type:medley', 'direction')
                ...
                if common_len > 0:
                    ...
                    if part_level > 0 and name_type == "work":
                        ...
                        for lev in range(
                            ...
                            if lev > 0:  # not filled_lowest and lev > 0:
                                ...
                                tm['~cwp' +
                                   ...
                                   '_part_' +
                                   str(lev -
                                           1)] = self.strip_parent_from_work(track, release_id, tm['~cwp' +
                                                                                                   meta_str +
                                                                                                   '_work_' +
                                                                                                   str(lev -
                                                                                                           1)],
                        ...
                        tm['~cwp' +
                            meta_str + '_part_' +
                            str(part_level - 1)] = \
                                self.strip_parent_from_work(track, release_id, tm[
                                    '~cwp' + meta_str + '_work_' +
                                    str(part_level - 1)], tm[
                                                                '~cwp' + meta_str + '_work_' +
                                                                str(part_level)], part_level - 1, False)[0]
                ...
                if 'arrangement' in self.parts[workId] and self.parts[workId]['arrangement'] \
                    ...
                    if extra_words:
                        if not diff and 'arrangement' in self.parts[
                            ...
                            strip = self.strip_parent_from_work(track, release_id, work, parent, part_level, False)
                else:
                    extend = True
                    strip = self.strip_parent_from_work(track, release_id, work, parent, part_level, extend, parentId)
    ...
    def make_annotations(self, release_id, track, wid):
        ...
        if options['cwp_key_include'] and 'key' in self.parts[wid] and self.parts[wid]['key']:
            ...
            write_log(release_id,'info', 'checking for key. keys = %s, names = %s', self.parts[wid]['key'], self.parts[wid]['name'])
            for key in self.parts[wid]['key']:
                if not any([key.lower() in x.lower() for x in str_to_list(self.parts[wid]['name'])]):
                    keys.append(key)
    ...
    def extend_metadata(self, release_id, top_info, track, ref_height, depth):
        ...
        if '~cwp_part_levels' not in tm:
            if self.DEBUG or self.INFO:
                write_log(release_id, 'debug', 'NO PART LEVELS. Metadata = %s', tm)
            return
        part_levels = int(tm['~cwp_part_levels'])
        if self.DEBUG or self.INFO:
            write_log(release_id, 'debug',
                ...
                if '~cwp_workid_' + str(lev) in tm:
                    ...
                    if 'medley_list' in self.parts[tup_id] and self.parts[tup_id]['medley_list']:
                        ...
                        if '~cwp_part_' + str(lev) in tm:
                            tm['~cwp_part_' + str(lev)] =  "(" + options["cwp_medley_text"] + ") " + tm['~cwp_part_' + str(lev)]
                ...
                if '~cwp_workid_' + str(lev) in tm:
                    ...
                    if 'annotations' in self.parts[tup_id]:
                        ...
                        if options["cwp_level0_works"] and '~cwp_X0_work_' + str(lev) in tm:
                            tm['~cwp_X0_work_' + str(lev)] += " (" + ', '.join(self.parts[tup_id]['annotations']) + ")"
                        if options["cwp_titles"] and '~cwp_title_work_' + str(lev) in tm:
                                tm['~cwp_title_work_' + str(lev)] += " (" + ', '.join(self.parts[tup_id]['annotations']) + ")"
                        if lev < part_levels:
                            if 'stripped_annotations' in self.parts[tup_id]:
                                if self.parts[tup_id]['stripped_annotations']:
                                    ...
                                    if options["cwp_level0_works"] and '~cwp_X0_part_' + str(lev) in tm:
                                        tm['~cwp_X0_part_' + str(lev)] += " (" + ', '.join(
                                            self.parts[tup_id]['stripped_annotations']) + ")"
                                    if options["cwp_titles"] and '~cwp_title_part_' + str(lev) in tm:
        ...
        if self.parts[interpret(tm['~cwp_workid_0'])
            ...
            if options["cwp_medley_text"]:
                groupheading =  groupheading + ' (' + options["cwp_medley_text"] + ')'


        write_log(release_id, 'info', 'groupheading check 3: %s', groupheading)
            ...
            if '~cwp_title_work_levels' in tm:
                ...
                for d in range(1, max_d):
                    ...
                    if tw_str in tm:
                        ...
                        work_main = work[d]
                        diff_work[d - 1] = self.diff_pair(release_id, track, tm, groupheading, title_work)
                        if diff_work[d - 1]:
                            diff_work[d - 1] = diff_work[d - 1].strip('.;:-,')
                            if diff_work[d - 1] == '…':
                                diff_work[d - 1] = ''
                        if d > 1 and tw_str_lower in tm:
                            title_part = self.strip_parent_from_work(track, release_id, tm[tw_str_lower], tm[tw_str], 0,
                                                                     False)[0]
                            if title_part:
                ...
                for w in range(ref_level - 2, -1, -1):
                    if diff_work[w] and diff_work[w+1]:
                        diff_work[w] = diff_work[w].replace(diff_work[w+1], '').strip(' .;:-,')
                        if diff_work[w] == '…':
                            diff_work[w] = ''
                if self.INFO:
            ...
            if '~cwp_part_levels' in tm:
                ...
                for n in range(0, part_levels + 1):
                    if '~cwp_work_' + \
                        ...
                        source_id = list(interpret(tm['~cwp_workid_' + str(n)]))
                        if n == 0:
                        ...
                        if n != 0 and n != part_levels:
                            self.append_tag(release_id, tm, 'musicbrainz_work_part_level' + str(n), source)
                            for source_id_item in source_id:
                                self.append_tag(release_id, tm, 'musicbrainz_work_part_level' + str(n) + '_id',
                                                source_id_item)
    ...
    def strip_parent_from_work(self, track, release_id, work, parent, part_level, extend, parentId=None):
        """
        Remove common text
        :param track:
        :param release_id: name for log file - usually =musicbrainz_albumid
        ...
        if self.DEBUG or self.INFO:
            ...
            write_log(release_id, 'info', 'PARAMS: WORK = %s, PARENT = %s, PART_LEVEL = %s, EXTEND= %s', work, parent, part_level, extend)
        if not isinstance(parent, str):
            # in case it is a list - make sure it is a string
            parent = '; '.join(parent)
        if not isinstance(work, str):
        ...
        if m:
            ...
            if extend and options['cwp_common_chars'] > 0:
                # try stripping out a common substring
                words = re.compile(r"\W?\b.*?\s|\W?\b.*$|\W?\B.*?\b|\W?\B.*?$")
                parent_words = words.findall(parent)
                clean_parent_words = parent_words[:]
                for w, word in enumerate(parent_words):
                    syn_word = self.synonymize(release_id, word, self.synonyms[track])
                    clean_parent_words[w] = self.boil(release_id, syn_word)
                work_words = words.findall(stripped_work)
                clean_work_words = work_words[:]
                for w, word in enumerate(work_words):
                    syn_word = self.synonymize(release_id, word, self.synonyms[track])
                    clean_work_words[w] = self.boil(release_id, syn_word)
                common_dets = longest_common_substring(clean_work_words, clean_parent_words)
                common_seq = common_dets['string']
                seq_length = common_dets['length']
                seq_start = common_dets['start']
                if self.INFO:
                    write_log(release_id, 'info',
                              'Checking common sequence between parent and work. ... parent_words = %s',
                              parent_words)
                    write_log(release_id, 'info', '... work_words = %s', work_words)
                    write_log(release_id, 'info', '... clean_parent_words = %s', clean_parent_words)
                    write_log(release_id, 'info', '... clean_work_words = %s', clean_work_words)
                    write_log(release_id, 'info', '... longest common sequence = %s', common_seq)
                if seq_length >= options['cwp_common_chars']:  # Make sure it is more than the required min
                    if seq_start > 0:
                        ellipsis = [u"\u2026" + ' ']
                    else:
                        ellipsis = []
                    stripped_work = ''.join(work_words[:seq_start] + ellipsis + work_words[seq_start + seq_length:]).lstrip(' :,-')
            ...
            if extend and parentId and parentId in self.works_cache:
                ...
                stripped_work = self.strip_parent_from_work(track, release_id, stripped_work, grandparent, part_level, True,
                                                            grandparentIds)[0]
    ...
    def diff_pair(self, release_id, track, tm, mb_item, title_item):
        ...
        if not mb:
            if self.INFO:
                write_log(release_id, 'info', 'End of DIFF_PAIR. Returning %s', None)
            return None
        ti = title_item.strip(" :;-.,")
        if ti.count('"') == 1:
            ti = ti.strip('"')
        ...
        #  synonyms
        mb_test = self.synonymize(release_id, mb, self.synonyms[track])
        ti_test = self.synonymize(release_id, ti, self.synonyms[track])
        ...
        if self.INFO:
            write_log(release_id, 'info', "Initial test. nopunc_mb = %s, nopunc_ti = %s, ti_len = %s", nopunc_mb,
                      nopunc_ti, ti_len)
        if self.INFO:
            write_log(release_id, 'info', "test sub....")
        lcs = longest_common_substring(nopunc_mb, nopunc_ti)['string']
        if self.INFO:
            write_log(release_id, 'info', "Longest common substring is: %s. ti_len is %s", lcs, ti_len)
        if len(lcs) >= ti_len:
            if self.INFO:
                write_log(release_id, 'info', 'End of DIFF_PAIR. Returning %s', None)
            return None
        ...
        if len(nopunc_mb) > 0:
            ti_new = self.strip_parent_from_work(track, release_id, ti_test, mb_test, 0, False)[0]
            # extend = False to ensure that partial elimination not done
            if ti_new == ti_test:
                mb_list = re.split(
                    r';\s|:\s|\.\s|\-\s', mb_test, 1)
                # was self.options[track]["cwp_granularity"] instead of just 1, but hardly used and doubtful value
                if self.INFO:
                    write_log(release_id, 'info', "mb_list = %s", mb_list)
                if mb_list:
                    for mb_bit in mb_list:
                        ti_new = self.strip_parent_from_work(track, release_id, ti_new, mb_bit, 0, False)[0]
                        if self.INFO:
                            write_log(release_id, 'info', "MB_BIT: %s, TI_NEW: %s", mb_bit, ti_new)
            else:
                if len(ti_new) > 0:
                    retn = self.reverse_syn(release_id, ti_new, self.synonyms[track])
                    if self.INFO:
                        write_log(release_id, 'info', 'End of DIFF_PAIR. Returning %s', retn)
                    return retn
                else:
                    if self.INFO:
                        write_log(release_id, 'info', 'End of DIFF_PAIR. Returning %s', None)
        ...
        if self.options[track]["cwp_split_hyphenated"]:
            regex = r"(?=\w*['])([\w']+)|(\b\w+\b)|(\B\&\B)"
            # allow ampersands and non-latin characters as word characters. Treat apostrophes as part of words.
            # TODO fix for right qupte marks as well as apostrophes?
        else:
            regex = r"(?=\w*['-])([\w'-]+)|(\b\w+\b)|(\B\&\B)"
            # allow ampersands and non-latin characters as word characters. Treat embedded hyphens and apostrophes as part of words.
        ti_tuple = tuple(re.finditer(regex, ti, re.UNICODE))  # to make it re-usable and countable
        ti_test_tuple = tuple(re.finditer(regex, ti_test, re.UNICODE))
        ...
        mb_test_tuple = tuple(re.finditer(regex, mb_test, re.UNICODE))
        mb_list2 = self.stencil(release_id, mb_test_tuple, mb_test)['match list']
        ...
        if s == 1:
            if 0 < index < len(ti_rich_list) - 1:
                # ignore singleton new words in middle of title unless they are within "cwp_end_proximity" from the start or end
                if ep < index < len(ti_rich_list) - ep - 1:
                    ti_rich_list[index] = (change, False)
                    s = 0
        if self.INFO:
            write_log(release_id, 'info', "ti_rich_list before gapping (True indicates a word in title not in MB work) = %s. length = %s", ti_rich_list,
                      len(ti_rich_list))
        if s > 0:
            d = p - ep
            ...
            for i, (ti_bit, new) in enumerate(ti_rich_list):
                if not new:
                    if self.INFO:
                        write_log(release_id, 'info', "item(i = %s) val = %s - not new. proximity param = %s, end_proximity param = %s", i, ti_bit, p, ep)
                    if start:
                        prox_test = ep
                    else:
                        prox_test = p
        ...
        if ti and len(ti) > 0:
            retn = self.reverse_syn(release_id, ti, self.synonyms[track])
            if self.INFO:
                write_log(release_id, 'info', 'End of DIFF_PAIR. Returning %s', retn)
            return retn
        else:
    ...
    def get_synonyms(self, release_id, track):
        tm = track.metadata
            ...
            if len(tup) == 2:
                for i, ts in enumerate(tup):
                    ...
                    if not tup[i]:
                        ...
                        if self.ERROR or self.INFO:
                            write_log(release_id, 'error',
                                      'Synonym keys cannot duplicate any in existing synonyms - error in %s '
                                      '- omitted from synonyms', syn)
                        self.append_tag(release_id, tm, '~cwp_error',
                                        '7. Synonym keys cannot duplicate any in existing synonyms - error in ' +
                                        syn + ' - omitted from synonyms')
                        tup[i] = "**BAD**"
                if "**BAD**" in tup:
                    continue
                else:
                ...
                if self.ERROR or self.INFO:
                    write_log(release_id, 'error', 'Error in synonym format for synonym %s', syn)
    ...
    def synonymize(self, release_id, str, synonyms):
        ...
        if self.INFO:
            write_log(release_id, 'info', 'Replaced Roman numerals. string = %s', str_test)
        for key, equiv in synonyms:
            ...
            str_test = re.sub(key_pattern, syno, str_test,
                             re.IGNORECASE | re.UNICODE)
            if self.INFO:
                write_log(release_id, 'info', "Replaced synonyms str_test = %s", str_test)
        return str_test
    ...
    def stencil(self, release_id, matches_tuple, test_string):
        ...
        pointer = 0
        if self.DEBUG or self.INFO:
            write_log(release_id, 'debug', 'In fn stencil. test_string = %s. matches_tuple = %s', test_string, matches_tuple)
        for match_num, match in enumerate(matches_tuple):
            start = match.start()
            end = match.end()
            if start > pointer:
                if pointer == 0:
                    match_items.append('')  # add a null word item at start to keep the lists the same length
                gap_items.append(test_string[pointer:start])
            else:
                if pointer > 0:
                    gap_items.append('') # shouldn't happen, but just in case there are two word items with no gap
            match_items.append(test_string[start:end])
            pointer = end
            if match_num + 1 == len(matches_tuple):
                gap_items.append(test_string[pointer:])  # pick up any punc items at end
        return {'match list': match_items, 'gap list': gap_items}


    def reverse_syn(self, release_id, term, synonyms):
        """
        reverse any synonyms left in the title item
        :param release_id: name for log file - usually =musicbrainz_albumid
        unless called outside metadata processor
        ...
        """
        if self.DEBUG or self.INFO:
            write_log(release_id, 'info', "REVERSING SYNONYM REPLACEMENT")
        for key, equiv in synonyms:
            if self.INFO:
                write_log(release_id, 'info', "equiv %s, key %s", equiv, key)
            equiv = self.EQ + equiv
            esc_equiv = re.escape(equiv)
            equiv_pattern = '\\b' + esc_equiv + '\\b'
        ...
        if self.INFO:
            write_log(release_id, 'info', "Returning term = %s",term)
        return term
...
class ClassicalExtrasOptionsPage(OptionsPage):
    ...
    def load(self):
        ...
        if 'ce_tab' in config.setting:
            cfg = config.setting.__dict__['_ConfigSection__config']['setting/ce_tab']  # seems to be the only way to get the value - config.setting['ce_tab'] returns None
    ...
    def save(self):
        ...
        # save tab setting
        config.setting['ce_tab'] = self.ui.tabWidget.currentIndex()  # works for setting, but not for getting
